...
| public **void** *SetInputBool*(**string** inputName, **bool** value) | Substance **Boolean**-Eingabe aktualisieren | **String** *inputName* Name der Eingabe im SBSAR **Bool** *value*-Wert, der zum Aktualisieren des Parameters verwendet wird |
| public **bool** *GetInputBool*(**string** inputName) | Substance **Boolean**-Eingabe abrufen | **Zeichenfolge** *Eingabename* Name der Eingabe in der SBSAR |
...
| **void Substance.Game.SubstanceGraph**.*SetInputTexture*( string inputName, Texture2D value) | Substance Texture2D-Eingabe aktualisieren | **String** *inputName* Name der Eingabe in SBSAR **Texture2D** *value*, die zum Aktualisieren des Parameters verwendet wird |
| **Texture2D Substance.Game.SubstanceGraph**.*GetInputTexture*( string inputName) | Substance Texture2D-Eingabe abrufen | **Zeichenfolge** *Eingabename* Name der Eingabe in der SBSAR |
| **VectorInt Substance.Game.SubstanceGraph**.*GetTexturesResolution*() | Texturauflösung der Zieleinstellungen des Diagramms abrufen (x = width, y = Height, Werte können 32, 64, 128, 256, 512, 1024, 2048 und 4096 sein) | Keine |
| **int Substance.Game.SubstanceGraph**.*SetTexturesResolution*( Vector2Int size) | Legen Sie die Texturauflösung für die Zieleinstellungen des Diagramms fest (Vector2Int’s x = width, y = Height, Werte können 32, 64, 128, 256, 512, 1024, 2048 &amp; 4096 sein) Gibt 0 zurück, wenn der Vorgang erfolgreich war, andernfalls: -1. | **Vector2Int** *size* zum Aktualisieren des Parameters verwendet&#x200B;**.** |
| **Liste Substance.Game.SubstanceGraph**.*GetGeneratedTextures*() | Gibt alle Substance Texture2D-Objekte zurück, die vom Material-Shader des Graphen verwendet werden. | Keine |
| **int Substance.Game.SubstanceGraph**.*Bake*( Texture2D texture, string absolutePath) | Generiert PNG-Dateien für alle Substance Texture2D-Objekte, die vom Material-Shader des Diagramms verwendet werden. | Keine |
| **&#x200B;**&#x200B;Substance.Game.**&#x200B; SubstanceGraph**.*Duplicate*() | Duplizieren eines Substance-Grafen | Keine |
| **Substance.Game.SubstanceGraph**.*Duplicate*(string newGraphName) | Dupliziere einen Substance-Grafen, und gib ihm einen Namen. (Das zugehörige Material erhält denselben Namen.) | **String newGraphName** |
| **&#x200B;**&#x200B;Substance.Game.**&#x200B; SubstanceGraph**.*GetInputProperties*() | Abfrage prozeduraler Eingabeinformationen, gibt ein Array von &quot;InputProperties&quot; zurück, mit :public Struktur InputProperties { public string name; // inputName public string label; // widget’s label in GUI public string group; // widget’s group in GUIpublic string[] componentLabels; // für Schieberegler (bis zu 4 Beschriftungen) public string[] enumOptions; // for optionMenuPublic InputPropertiesType type;public Vector4 maximum; // für Regler public Vector4 minimum; // für Regler public float step; // für Regler }public enum InputPropertiesType { Boolean = 0,// 0 Float, // 1 Vector2, // 2 Vector3, // 3 Vector4, // 4 Color, // 5 Enum, // 6 Texture, // 7 String, // 8 Invalid = -1// -1 }; | Keine |
| **bool** **Substance.Game.SubstanceGraph**.*HasInput*(**string** inputName) | Überprüfen Sie, ob eine Eingabe in einem Diagramm vorhanden ist, gibt &quot;true/false&quot; zurück: | **Zeichenfolge** *Eingabename* Name der Eingabe in der SBSAR |
...
| Public-Methode | Beschreibung | Parameter |
| --- | --- | --- |
| public **void** *QueueForRender*() | Substance-Diagramm zur Warteschlange hinzufügen | Keine |
| ***mySubstance.**&#x200B;RenderAsync()* | Alle in die Warteschlange gestellten Substance-Graphen asynchron rendern | Keine |
| ***mySubstance.**&#x200B;RenderSync()* | Alle in die Warteschlange gestellten Substance-Graphen synchron rendern | Keine |

## Scripting im Editor-Modus:

Damit Diagrammänderungen im Editor-Modus permanent vorgenommen werden, muss ein erneuter Import der jeweiligen Substance durchgeführt werden. Dies geschieht mit der folgenden Funktion:
...
(mit &quot;CommitSubstanceToImporter&quot;, einer Substance-Plug-in-Funktion: Kopieren aller modifizierten Diagrammparameter und/oder Eingaben in das Substance-Importobjekt, das dann über den Importmechanismus von Unity auf die Festplatte serialisiert wird.)
