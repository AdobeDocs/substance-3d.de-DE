---
helpx_url: "https://helpx.adobe.com/de/substance-3d-integrations/3d-applications/blender/troubleshooting.html"
breadcrumb-title: ''
...
* *Probleme bei der Installation des Add-Ons./ Materialien bleiben bei der Verarbeitung zwischen den Sitzungen hängen. / Materialien erzeugen keine Texturen zwischen Sitzungen. / Fehler beim Laden von .sbsar-Dateien.*
...
* *Materialien werden in der Zyklen-Renderansicht nicht aktualisiert*.
  * Standardmäßig aktualisiert das Add-on die Texturen in der Renderansicht &quot;Zyklen&quot; nicht. Sie können jedoch durch Aktivierung von <b>Zyklen Texturen automatisch aktualisieren</b> in den Add-On-Voreinstellungen aktualisiert werden.
* Parameter scheinen nach dem Speichern während der Zyklen-Renderansicht zurückgesetzt zu werden.
  * Dies ist ein bekanntes Caching-Problem auf der Mischerseite, das nur visuell angezeigt wird. Beim Speichern wird keine Meldung zum Aktualisieren der generierten Texturdateien an die Remote-Engine gesendet. Die Texturen werden normal angezeigt, nachdem Sie die Zyklen-Renderansicht verlassen und wieder zur Ansicht gewechselt sind.
* *Materialien werden nach dem Rückgängigmachen/Ändern von Parametern nicht mehr aktualisiert.*
  * Materialien können möglicherweise nicht aktualisiert werden, nachdem Aktionen rückgängig gemacht wurden. Die Parameter werden in den vorherigen Zustand zurückgesetzt, die Texturen werden jedoch nicht rückgängig gemacht. Wenn Sie die Textur erneut aktualisieren möchten, verwenden Sie die Schaltfläche &quot;Aktualisieren&quot;, um die Parameter auf die Standardeinstellungen zurückzusetzen und die Texturen neu zu laden.
* *Die im Substance Designer festgelegten Farben werden im Farbwähler des Mischers leicht unterschiedlich angezeigt, und die Farbwerte sind nicht identisch.*
  * Der Mischer wendet eine Gamma-Korrektur nur auf Farben für den Farbwähler des Mischers an. Dies verursacht zwar eine Diskrepanz im Farbwähler, aber die Farben, die in Texturen angezeigt werden, stimmen mit den in Substance-Apps festgelegten Werten überein.
* *Konsolenfehler &quot;wmic is not detected&quot; (wmic wird nicht erkannt) beim Laden eines Materials in Windows.*
